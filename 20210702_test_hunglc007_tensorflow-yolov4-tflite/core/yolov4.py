...
def compute_loss(pred, conv, label, bboxes, STRIDES, NUM_CLASS, IOU_LOSS_THRESH, i=0):
    ...
    # # kjw
    # import numpy
    # if numpy.isnan(giou_loss):
    #     pass
    # if numpy.isnan(conf_loss):
    #     conf_loss = tf.constant(1e-10, dtype=float, shape=1)
    # if numpy.isnan(prob_loss):
    #     prob_loss = tf.constant(1e-10, dtype=float)

    # giou_loss = tf.constant(9.198023/3, dtype=float, trainable=True)
    # conf_loss = tf.constant(1e-10, dtype=float, shape=-1)
    # prob_loss = tf.Variable(1e-10, dtype=float, trainable=True)
