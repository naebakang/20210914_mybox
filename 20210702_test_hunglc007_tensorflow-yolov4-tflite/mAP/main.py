...
dir_output = ins_env_directory.project_output_ground + r'/mAP/results'
...
parser.add_argument('-o', '--output', default=r"{}".format(dir_output), type=str, help="output path name")
...
img_path = ins_env_directory.test_data
...
ground_truth_files_list = glob.glob('{}/mAP/ground-truth/*.txt'.format(ins_env_directory.project_output_ground))
...
for txt_file in ground_truth_files_list:
  ...
  if not os.path.exists('{}/mAP/predicted/'.format(ins_env_directory.project_output_ground) + file_id + ".txt"):
  ...
  for line in lines_list:
    try:
      if "difficult" in line:
          class_name, x_center, y_center, width, height,  _difficult = line.split()
          is_difficult = True
      else:
...
predicted_files_list = glob.glob('{}/mAP/predicted/*.txt'.format(ins_env_directory.project_output_ground))
...
for class_index, class_name in enumerate(gt_classes):
  ...
  for txt_file in predicted_files_list:
    ...
    for line in lines:
      try:
        tmp_class_name, confidence, top, left, bottom, right = line.split()
      except ValueError:
        error_msg = "Error: File " + txt_file + " in the wrong format.\n"
  ...
  for class_index, class_name in enumerate(gt_classes):
    ...
    for idx, prediction in enumerate(predictions_data):
      ...
      if show_animation:
        ...
        ground_truth_img = glob.glob1(img_path, file_id + ".*")  # kjw
        a = []
        for i in ground_truth_img:
          if i[-1] == 't':
            pass
          else:
            a.append(i)
        ...
        if len(ground_truth_img) == 0:
          ...
          img_kjw = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
          image_h, image_w, _ = img_kjw.shape  # kjw
          resize_x = 416  # kjw
          resize_y = int(resize_x/image_w*image_h)  # kjw
          img = cv2.resize(img, dsize=(resize_x, resize_y), interpolation=cv2.INTER_AREA)  # kjw
          image_h, image_w, _ = img.shape  # kjw
      ...
      for obj in ground_truth_data:
        ...
        if obj["class_name"] == class_name:
          ...
          bbgt = change_coordinate.get_min_max_coor_from_xywh(bbgt)
          bbgt[0] = int(bbgt[0] * image_h)  # kjw
          bbgt[2] = int(bbgt[2] * image_h)  # kjw
          bbgt[1] = int(bbgt[1] * image_w)  # kjw
          bbgt[3] = int(bbgt[3] * image_w)  # kjw

          bi = [max(bb[1],bbgt[1]), max(bb[0],bbgt[0]), min(bb[3],bbgt[3]), min(bb[2],bbgt[2])]
          iw = bi[3] - bi[1] + 1
          ih = bi[2] - bi[0] + 1
          if iw > 0 and ih > 0:
            # compute overlap (IoU) = area of intersection / area of union
            ua = (bb[3] - bb[1] + 1) * (bb[2] - bb[0] + 1) + (bbgt[3] - bbgt[1]
                    + 1) * (bbgt[2] - bbgt[0] + 1) - iw * ih
            ov = iw * ih / ua
            if ov > ovmax:
              ovmax = ov
              gt_match = obj
      ...
      if show_animation:
        ...
        if ovmax > 0: # if there is intersections between the bounding-boxes
          bbgt = [ float(x) for x in gt_match["bbox"].split() ]  # kjw
          bbgt = change_coordinate.get_min_max_coor_from_xywh(bbgt)
          bbgt[0] = int(bbgt[0] * image_h)  # kjw
          bbgt[2] = int(bbgt[2] * image_h)  # kjw
          bbgt[1] = int(bbgt[1] * image_w)  # kjw
          bbgt[3] = int(bbgt[3] * image_w)  # kjw
          bbgt = bbgt.astype(int)
          cv2.rectangle(img,(bbgt[1],bbgt[0]),(bbgt[3],bbgt[2]),light_blue,2)
          cv2.rectangle(img_cumulative,(bbgt[1],bbgt[0]),(bbgt[3],bbgt[2]),light_blue,2)
          cv2.putText(img_cumulative, class_name, (bbgt[1],bbgt[0] - 5), font, 0.6, light_blue, 1, cv2.LINE_AA)
        bb = [int(i) for i in bb]
        cv2.rectangle(img,(bb[1],bb[0]),(bb[3],bb[2]),color,2)
        cv2.rectangle(img_cumulative,(bb[1],bb[0]),(bb[3],bb[2]),color,2)
        cv2.putText(img_cumulative, class_name, (bb[1],bb[0] - 5), font, 0.6, color, 1, cv2.LINE_AA)
        # show image
